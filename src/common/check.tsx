...
export default class Check extends WeElement<CheckProps> {
  render(props: OmiProps<CheckProps, any>, store: any) {
    ...
    const labelClassName = classNames(TdClassNamePrefix(`${type}`), this.className, {
      [TdClassNamePrefix(`is-checked`)]: internalChecked,
      [TdClassNamePrefix(`is-disabled`)]: disabled,
      [TdClassNamePrefix(`is-indeterminate`)]: indeterminate,
    })
    ...
    const input = (
      <input
        ...
        class={classNames(TdClassNamePrefix(`${type}__former`))}
    ...
    return (
      ...
      >
        {input}
        <span class={classNames(TdClassNamePrefix(`${type}__input`))} />
        {showLabel && (
          <span key="label" class={classNames(TdClassNamePrefix(`${type}__label`))} onClick={handleLabelClick}>
            {children || label}
          </span>
        )}
      </label>
